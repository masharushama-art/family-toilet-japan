...
### Q4. パフォーマンス計測と改善 ⏸ 計測待ち
- PageSpeed Insights APIは無キー枠の日次クォータ超過で計測不可だった（2026-07-04）
- **ユーザー実行**: [pagespeed.web.dev](https://pagespeed.web.dev) で `family-toilet-japan.vercel.app/` と `/map` をモバイル計測し、スコアを教えてください。それを基に改善を実施
