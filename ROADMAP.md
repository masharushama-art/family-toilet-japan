...
### Q3. サイト内検索（インパクト中・工数小）
現状、地図ページ内の絞り込み検索しかなく、トップページから「施設名・駅名・エリア名」で直接探す手段がない。
- トップページに検索ボックスを設置、既存の全トイレJSON＋スポット＋ガイドを対象にクライアントサイド検索（Fuse.js等の軽量ライブラリか自前実装）
- 検索結果からトイレ個別・スポット・ガイドへ直接遷移
- 効果: 直帰率低下・回遊性向上。GA4のsearchイベントで「ユーザーが何を探しているか」の生データも取れる
