...
最終更新: 2026-09-26（このファイルは実装のたびに更新する）
...
**実装**: `.github/workflows/monthly-data-refresh.yml`に以下を追加。
1. `scripts/check-data-sanity.mjs`（新規）: `data-history.json`の最新2件を比較し、全国合計の変化が-3%〜+8%の範囲外、またはいずれかの都市が-30%以上減少していたら`safe=false`を出力（Overpass APIの一時的な取得失敗や重複バグを想定した閾値）。ローカルで正常ケース（+0.1%→safe=true）と異常ケース（都市データ消失を模擬→safe=false・理由付きで検出）の両方を確認済み
2. `npm run build`によるビルドチェックを追加
3. sanity checkとビルドの両方が通った場合のみ`gh pr merge --squash --delete-branch`で自動マージ。どちらか一方でも失敗した場合はPRを開いたままにし、sanity check失敗時はその理由をPRにコメントする

**注意点（既知の制約）**: 自動マージ後もCloudflareへの本番デプロイは引き続き`npm run cf:deploy`の手動実行が必要（このプロジェクトはpush時自動デプロイなしの方針、CLAUDE.md参照）。また`GITHUB_TOKEN`によるマージは`push`トリガーの`indexnow.yml`を起動しない可能性がある（GitHub Actionsの仕様上、GITHUB_TOKEN発のイベントは他ワークフローの再トリガーに使われないことがある）ため、月次更新後のIndexNow通知は今まで通りデプロイ後に成り行きで届くか確認が必要。次回実行（2026-10-01、または`workflow_dispatch`での手動テスト実行）で実際に自動マージが機能するか確認する。

## ✅ 都市・カテゴリページのtitle/description CTR改善（2026-09-26、本番反映済み）

Search Console検索パフォーマンス（2026-07-14〜09-24、3ヶ月集計）を確認したところ、全体の平均掲載順位は19.1（前回ログ2026-09-23時点の62.3から大幅改善）まで来ている一方、直近6週間はクリックがほぼ0で横ばい。クエリ別に見ると「toilet near me」（掲載順位7.3・表示回数48）「public toilet near me」（7.5）「toilets near me」（7.0）などの近隣検索クエリがいずれもCTR 0%だった。ページ別内訳では`/kyoto/free`・`/nagoya`・`/hiroshima`・`/yokohama`などの都市・カテゴリページがこれらのクエリで表示されており、掲載順位は悪くないのに"near me"の検索意図に応えるtitle/descriptionになっていなかったことが原因と判断した。

**対応**: `app/[city]/page.tsx`・`app/[city]/[category]/page.tsx`のgenerateMetadataを"near you"の意図に応える文言に変更。
- 都市ページ: `Family Friendly Toilets in {city}, Japan` → `Toilets Near You in {city}, Japan — {N}+ Locations`
- カテゴリページ（おむつ交換台・車椅子対応・無料トイレ、3種共通テンプレート）: `{カテゴリ名} in {city} Japan — {N} locations` → `{カテゴリ名} Near You in {city} — {N} Locations`。descriptionも「最寄りの◯◯が見つかる」という導線に統一

なお日本語の駅名クエリ（曙橋 トイレ・学習院下 トイレ等）は掲載順位26〜46位と低く、しかもヒットしているのがZH版スポットページ（`/zh/spot/akebonobashi-station`）だったため、meta文言だけでの改善効果は薄いと判断し今回は対象外とした。

**本番反映（2026-09-26）**: コミット`555708c`をpushし、`npm run cf:deploy`でデプロイ（Version ID `ea5dfb57-c211-4260-a37f-333f3b34ed44`）。本番`/kyoto/free`・`/tokyo`・`/nagoya`で新しいtitleが出力されること、`/coverage`のDataset構造化データに回帰がないことを確認。効果測定は次回月次ログ（2026-10-01頃、Search Consoleのクリック数・CTRの変化）で確認する。
