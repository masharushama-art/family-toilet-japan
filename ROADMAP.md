...
- AdSense: **4回連続不承認**（07-05・08-14・08-27・09-23、すべて「有用性の低いコンテンツ」、文言は毎回同一の定型文で個別ページの指摘なし）。2026-09-06にトイレ個別ページを全件noindex化（第1弾）、共通ヘッダー/フッター新設・薄いページのnoindex・Consent Mode v2・運営者情報の構造化データ（第2弾）、EN都市ページの残存重複解消（第3弾）、JA都市ページへのTips追加（09-15）、http→https 301リダイレクト有効化（09-21）を実施し、2026-09-13に4回目再審査を申請したが09-23付で再び不承認。詳細は下記「4回目のAdSense再審査 結果（不承認）」参照。今後の方針は「🧭 今後の方針」（再申請凍結・アフィリエイト先行・実績づくり、2026-09-23承認済み）参照
