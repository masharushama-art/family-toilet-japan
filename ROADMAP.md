...
### 5. 季節特集の残り
- ✅ 紅葉（koyo）特集: 完了（2026-07-03）。`/guide/autumn-foliage-with-kids-japan`、4言語対応。時期の見極め・ベビーカー向けスポット・防寒対策・トイレ探しの4セクション構成。sitemap・各言語トップページに登録済み。ついでにEN版トップページから漏れていた夏祭り特集へのリンクも追加
...
1. **公共交通オープンデータセンター（ODPT）** — 鉄道各社の駅施設API。開発者登録申請済み（2026-07-03、承認まで最大2営業日）。**駅データの本命**
2. 自治体オープンデータの公衆トイレCSV（中核市クラスに未取り込みが多数）
3. 国交省「歩行空間ネットワークデータ」（バリアフリー経路）
