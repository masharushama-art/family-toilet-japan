...
最終更新: 2026-09-16（このファイルは実装のたびに更新する）
...
- AdSense: 3回連続不承認（07-05・08-14・08-27、いずれも「有用性の低いコンテンツ」）。**2026-09-06にトイレ個別ページを全件noindex化（第1弾）し、さらにサイト全体監査で共通ヘッダー/フッター新設・薄いページのnoindex・Consent Mode v2・運営者情報の構造化データ等を実施（第2弾）**。インデックス対象は181 URL（ガイド78・都市94・静的/言語9）。**2026-09-13、4回目の再審査を申請済み（下記「4回目のAdSense再審査申請」参照）。結果待ち**
...
### 📝 メモ: `/map`（bare、クエリなし）のコンテンツの薄さについて（2026-07-28）
`/map?id=...&city=...`形式のクエリ付きページはGSCの重複指摘を受けてnoindex化した（別項）。一方、クエリなしのbare `/map`自体のコンテンツの薄さ（地図表示のみで固有の解説文等はない）については、今回はスコープ外とした。トップ・全ガイド・FAQ・aboutなど15箇所以上から内部リンクされる主要な入口であるため、noindex化は影響範囲が大きく慎重に判断すべき論点として残す。次回の薄いページ対策見直しの際に検討する。
