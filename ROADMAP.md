...
### Q4. パフォーマンス計測と改善 🔄 第1弾実施済み（2026-07-04）
**計測結果（モバイル）**: トップ 74点 / `/map` **44点**（LCP 13.9s・TBT 1,560ms・CLS 0）
**実施した改善（/map対象）**:
- Esriタイルサーバーへの `preconnect`（LCPタイルのDNS+TLS先行確立）
- 都市トイレJSONを `priority: "low"` でfetch（低速回線でタイル画像に帯域を譲る）
- `/data/*`・`/icons/*` に長期Cache-Controlヘッダー（PSI指摘の203KiB対策）
**次回**: デプロイ後に再計測して効果確認。残る候補: 未使用JS 326KiB削減（バンドル分析）、メインスレッド3.3s（長時間タスク8件）のマーカー生成チャンク化、画像配信332KiB
**副産物の発見**: 現行データには`ostomate`タグ付きトイレがほぼ存在しない（フィルターをONにすると0件になる）。月次データ更新で取得タグに追加済みなので、次回更新後に再確認。それまでostomateフィルターは実質機能しない点に注意
