  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <ol className="text-sm text-gray-600 space-y-2 mb-8 list-decimal list-inside">
        ...
        <h2 className="text-xl font-bold text-gray-800 mb-4">Embed codes</h2>
        <div className="space-y-4 mb-10">
          {EXAMPLE_CITIES.map((city) => (
            <div key={city} className="border border-gray-100 rounded-xl p-4">
              <p className="font-semibold text-gray-800 text-sm mb-2">
                {CITIES[city].name}
              </p>
              <pre className="bg-gray-50 rounded-lg p-3 text-xs text-gray-700 overflow-x-auto whitespace-pre-wrap break-all">{snippet(city)}</pre>
            </div>
          ))}
          <p className="text-xs text-gray-400">
            Works for all 47 prefectures — just change the <code className="bg-gray-100 px-1 rounded">city=</code> parameter to any city slug (e.g. nagoya, hiroshima, okinawa).
          </p>
        </div>
        ...
        <div className="bg-sky-50 rounded-2xl p-5 mt-8 text-sm text-sky-800">
