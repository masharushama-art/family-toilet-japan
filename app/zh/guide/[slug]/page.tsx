...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  return {
    ...
    keywords: g.keywords,
    alternates: {
      canonical: `${BASE}/zh/guide/${slug}`,
      languages: {
        en: `${BASE}/guide/${slug}`,
        "zh-TW": `${BASE}/zh/guide/${slug}`,
      },
    },
  };
}

export default async function ZhGuidePage({ params }: { params: Params }) {
  const { slug } = await params;
  const g = ZH_GUIDES[slug];
  if (!g) notFound();
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <div className="max-w-2xl mx-auto px-6 pt-4">
        <ShareButtons url={`${BASE}/zh/guide/${slug}`} text={g.title} imageUrl={`${BASE}/zh/guide/${slug}/opengraph-image`} />
      </div>
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <HotelAffiliateBox cityNameJa={(() => { const c = getCityForGuideSlug(slug); return c ? CITIES[c as CitySlug].jaName : ""; })()} lang="zh" />
        ...
        <div className="mb-8">
          <h2 className="font-bold text-gray-800 dark:text-gray-100 mb-3 text-sm">其他指南</h2>
          <div className="flex flex-wrap gap-2">
            {ZH_GUIDE_SLUGS.filter((s) => s !== slug).map((s) => (
              <Link
                key={s}
                href={`/zh/guide/${s}`}
