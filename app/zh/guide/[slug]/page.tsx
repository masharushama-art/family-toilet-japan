    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <div className="bg-gray-50 rounded-2xl p-6 mb-8">
          ...
          <p className="text-sm text-gray-600 mb-4">{g.mapDesc}</p>
          <Link
            href={g.mapHref}
        ...
        <div className="mb-8">
          <h2 className="font-bold text-gray-800 mb-3 text-sm">其他指南</h2>
          <div className="flex flex-wrap gap-2">
            {ZH_GUIDE_SLUGS.filter((s) => s !== slug).map((s) => (
              <Link
                key={s}
                href={`/zh/guide/${s}`}
                className="text-xs bg-sky-50 text-sky-700 px-3 py-1.5 rounded-full hover:bg-sky-100 transition-colors"
              >
                {ZH_GUIDES[s].title}
              </Link>
            ))}
